...
public:
    int count(int D, int S) {
        ...
        rep3(i, 1, 10) dp[(1<<i)*(1<<10)+(1<<i)] = 1;
        rep(i, D-1) {
            ndp.assign(1<<20, 0);
            rep3(j1, 1, 1<<10) rep3(j2, 1, 1<<10) if (dp[j1*(1<<10)+j2] != 0) {
                rep(k, 10) ndp[ito[j1][k]*(1<<10)+dto[j2][k]] = (ndp[ito[j1][k]*(1<<10)+dto[j2][k]] + dp[j1*(1<<10)+j2]) % mod;
            }
            swap(ndp, dp);
        }
        int res = 0;
