...
int main() {
    ...
    rep(i, n) if (!xlst[i].empty()) {
        for (auto& pi : xlst[i]) dc.cut(pi.first, pi.second);
        for (auto& pi : xlst[i]) res += (ll)dc.size(pi.first) * dc.size(pi.second);
        for (auto& pi : xlst[i]) dc.link(pi.first, pi.second);
    }
    cout << res << endl;
    return 0;
}
